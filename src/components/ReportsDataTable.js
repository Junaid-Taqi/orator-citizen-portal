...
const ReportsDataTable = ({ reports }) => {
    ...
    const getPriority = (id) => {
        const p = id % 3;
        if (p === 0) return { label: 'High', class: 'badge fs-12 border border-danger text-danger bg-transparent rounded-pill px-3' };
        if (p === 1) return { label: 'Medium', class: 'badge fs-12 border border-warning text-warning bg-transparent rounded-pill px-3' };
        return { label: 'Low', class: 'badge fs-12 border border-info text-info bg-transparent rounded-pill px-3' };
    };
